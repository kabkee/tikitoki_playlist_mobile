...
export const useDataStore = defineStore("data", () => {
	...
	const videoList_org = ref(VIDEO_LIST);
	...
	const videoRepeatOnce = ref(false);
	const videoList = computed(() => {
		// make a function, which
		// depends on the videoRepeatOnce's value, the item's "videoUrl" attribute value is changed.
		// if it's true and the videoUrl contains 'loop=0', it's replaced the to "loop=1"
		// if it's false and the videoUrl contains 'loop=1', it's replaced the to "loop=0"

		let newVideoList = [...videoList_org.value];
		videoList_org.value.forEach((item, itemIdx) => {
			if (videoRepeatOnce.value) {
				newVideoList[itemIdx].videoUrl = item.videoUrl.replace(
					"loop=0",
					"loop=1"
				);
			} else {
